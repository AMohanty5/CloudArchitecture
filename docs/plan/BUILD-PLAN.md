...
- [x] **54** ✅ (2026-06-21) Relationship editing UX: **drop a service onto a component to attach it** (`addComponentLinkedTo` creates the classified relationship → it folds), and an **Inspector relationship panel** listing Attachments / Security / Identity / Communicates-with with a **✕ Detach** (un-folds / removes the line) — makes folded resources manageable again. Pure `groupRelationships()` helper (2 tests). *Deferred to Day 55: VPC endpoints + an explicit "+ Attach…" picker.*
...
- [x] **58** ✅ (2026-06-21) Container demotion — structural containers (region/VPC/subnet) now render as a **faint `CONTAINER.wash`** + a **1px hairline** (`CONTAINER.borderAlpha`) + a **quiet corner label** (no full-width header bar, no bottom border, no `region`/`network`/`subnet` kind suffix); flat (no shadow). Region label larger + lighter (`TYPE_SCALE.region`). Tier **section panels keep their intentional panel look**. Security 🛡 chips + finding/invalid indicators preserved. 95 web tests + typecheck green.
- [x] **59** ✅ (2026-06-21) Subnet lanes — public vs private subnets now read as distinct **sky** vs **slate** bands (`SUBNET_TINT`, public/private wash + accent + an uppercase `public`/`private` corner tag), driven by `properties.public` carried through the projector (defaults private). +1 projector test (96 web). *The full backdrop-lane render that decouples routing from nesting moves to Day 62 (layout engine).*
- [x] **60** ✅ (2026-06-21) **Full dark theme across surfaces** — Canvas sets CSS custom properties (`--cac-surface/text/muted/hairline/ring`) from `CANVAS_THEME[theme]` on the wrapper; they cascade to every custom node/container (no theme threading through the pure projector). ServiceNode + GroupNode (cards, section panels, labels, chips, text, borders) now read `var(--cac-*)`, so the ☀/🌙 toggle reaches all surfaces, not just the backdrop. 96 web tests + typecheck green. *Deferred: edge/connector dark colors → Day 61; legend/title overlay theming → Day 65; region/VPC backdrop-layer + z-order → Day 62.*
- [x] **61** ✅ (2026-06-21) Connector restyle — edges now desaturate to the theme connector palette (only **traffic**=accent + **data**=accent keep colour; async/replication/peering/etc go **neutral**, told apart by dash), are **thinner** (1.5px base, was 1.75) with **smaller arrowheads** (16→11), and are **theme-aware** (dark uses the brighter `CANVAS_THEME.dark.connector`; labels use themed surface/muted). Legend (`CONNECTOR_KINDS`) realigned to match. 96 web tests + typecheck green. *Port-side routing constraints move to Day 62 (layout engine).*
- [x] **62** ✅ (2026-06-21) Layout polish — `fitView` fill (`padding 0.18, maxZoom 1.4`) centres + frames with a consistent margin so a small diagram no longer floats right with dead canvas; ELK base tuned for whitespace (`nodeNodeBetweenLayers 40→56`, `nodeNode 20→28`) + `NETWORK_SIMPLEX` placement for straighter edges/fewer bends. 96 web tests + typecheck green. *Deferred (the deep, risky pieces — kept out of a pre-deploy day): virtual **Internet entry node**, **flow-rank decoupled from subnet nesting**, and the **region/VPC backdrop layer** → folded into Days 63–64 (archetypes, where flow direction is the focus) / backlog.*
- [x] **63** ✅ (2026-06-21) Architecture flow origin — the projector synthesizes a single **"🌐 Internet" entry node** + traffic edges to every internet-facing entry point (CloudFront / API Gateway / IGW / internet-facing LB / Global Accelerator), so a diagram reads left-to-right from a clear start. Projection-only (never persisted); new `EntryNode` (themed pill) + `entry` node type. +2 projector tests (98 web); existing fixtures unaffected (no internet-facing endpoints). *Per-archetype lane tuning (3-Tier / Multi-AZ / Serverless / Event-driven tier-ranks) → Day 64.*
- [x] **64** ✅ (2026-06-21) Archetype flow ordering — opt-in **"Flow ⇢"** layout preset that partitions nodes by `tierRank` (entry 0 → edge 1 → compute 2 → data 3) via ELK partitioning, so a diagram reads Internet→edge→compute→data left-to-right. Default layout unchanged (opt-in via the toolbar preset dropdown). Best for flatter archetypes (serverless / event-driven / data-pipeline). +2 layout tests (100 web). *Named-archetype auto-detection + Microservices/Hub-spoke/GenAI presets → backlog; full flow-rank-vs-nesting decoupling still needs the region/VPC backdrop-layer (backlog).*
- [x] **65** ✅ (2026-06-21) Overlay theming — the **legend** and **title block** now read the `--cac-*` theme vars (surface/hairline/text/muted) so they're dark panels in dark mode instead of bright white (completes the dark theme: pane, nodes, containers, edges, and now overlays). Lightweight SG/IAM/NACL/endpoint indicators already shipped as chips/badges (Days 53/55). 100 web tests + typecheck green.
- [x] **66** ✅ (2026-06-21) Golden review (projection-level; pixel goldens need a browser) — `templates.golden.test.ts` renders all 6 templates and asserts Phase-2 invariants: no dangling edges after fold/entry, nesting order, and **every component is represented** (node / section row / fold badge). That last check **caught a real bug** — folding a relationship whose owner was a tier section-panel row made the secondary vanish (KMS/Secrets in `layered-platform`); fixed with an `inSection` guard so folding is skipped for panel members. 20 golden tests (120 web). **Export parity:** PNG rasterises the live canvas → exact; **SVG renderer (`apps/core/.../diagram/svg.ts`) is now materially behind** (no folding/lanes/entry/washes) → tracked as a scoped follow-up (Backlog).
...
- [~] **70** ◑ (2026-06-21) **Composed render mode (beta, flagged)** — `project(model, layout, { compose })` + `composeProjection`: leaf nodes go flat (absolute coords, no parentId) and structural region/VPC/AZ/subnet boxes are replaced by `computeBackdrops` rectangles behind the nodes; section panels kept. New **▢ Compose** toolbar toggle (persisted, default off) → first eyeball-able backdrop view. Default nested render untouched. +2 projector tests (127 web). *Still deferred (the can't-verify-blind part): flat-ELK flow-rank decoupling to minimise connector length — compose currently reuses the nested layout's positions. Revisit after eyeballing.*
- [x] **71** ✅ (2026-06-21) **AZ container layer** — `zone` (AZ) nests `region ⊃ vpc ⊃ az ⊃ subnet` (containment: subnet ⊂ [network, zone], zone ⊂ network); AZ renders **lighter than VPC** (dashed slate band, fainter wash); backdrops are depth-generic so AZ enclosure works automatically. Multi-AZ HA template re-scaffolded (subnets under `az-a`/`az-b`). **Synthetic Region + Availability Zone palette items** (no catalog service → drop creates the group). +3 tests (130 web).
- [x] **72** ✅ (2026-06-21) **Subnet role awareness** — `subnets.ts`: a subnet's role (web/app/data/shared/management/transit) comes from `properties.role` or is **inferred from members** (edge→web, compute→app, datastore→data); the lane tag now reads e.g. `PRIVATE · APP`; `roleTier` orders lanes web→app→data for the flow layout. Wired through the projector + `computeBackdrops`. +4 tests (134 web).
- [x] **73** ✅ (2026-06-21) **Network-link folding** — a peering / TGW / VPN / Direct-Connect component joining **exactly two** network groups folds into a single labeled connector between them (`VPC A ◄══ peered ══► VPC B`, bidirectional) and the box disappears; a link joining **>2** VPCs stays a **hub node** (a transit gateway is genuinely a hub). Works in nested + compose modes. +2 projector tests (136 web).
- [x] **74** ✅ (2026-06-21) **Networking placement** — **NET-002** validation: a gateway endpoint inside a subnet warns (it's a route-table target → belongs at VPC level; the inverse of NET-001). **NACL is already a VPC-level construct** via the generic group-fold: connecting a NACL to a VPC folds it as a 🛡 VPC chip, and connecting it to multiple subnets folds a chip onto **each** (subnet associations) — both now covered by projector tests. +4 tests (138 web · 26 validation).
- [x] **75** ✅ (2026-06-21) **Category treatments** — **observability sidecars**: a new `monitors` relationship class (CloudWatch/X-Ray ↔ compute/db/storage/LB) folds the observability resource onto the watched node as a 📊 sidecar chip (alerting → messaging stays a flow); new `sidecar` fold bucket threaded through projector + ServiceNode + inspector. **Active-path highlight**: selecting a node emphasises its incident edges and dims the rest (opacity 0.28). +3 tests (140 web). *Deferred: networking overlays (gateway-endpoint chip — placement done Day 74, overlay render → backlog); further card compaction (already 46px from Day 57) → backlog.*
- [x] **76** ✅ (2026-06-21) **Architecture layer bands** — `layers.ts` `architectureLayer(type)` classifier + `projectLayers` band-layout view: the whole model reorganizes into horizontal **EDGE → NETWORK → COMPUTE → INTEGRATION → DATA → SECURITY → OBS** bands (infra-agnostic), drawing only the communication flow between bands. New **▦ Layers** toolbar toggle (persisted). SG/NACL classed to SECURITY (per the solution-architecture layer model). +4 tests (145 web).
- [x] **77** ✅ (2026-06-21) **Architecture view system** — pure `applyView(model, view)` (`views.ts`) generates 4 views from one model: **Resource** (editable), **Architecture** (hides security/observability detail), **Network** (only networking + SG/NACL), **Executive** (aggregates to Users → Application → Data Platform, infra hidden). Toolbar **👁 view dropdown**; non-Resource views are read-only display abstractions (editing props gated off). +4 tests (149 web). *Composes with the ▢ Compose / ▦ Layers render modes.*
- [x] **78** ✅ (2026-06-22) Golden review — `composition.golden.test.ts` runs all 6 templates through the Phase-2C machinery: **compose** (every backdrop encloses its members — cohesion), **layers** (every component banded, edges stay within bands), **views** (architecture/network/executive have no dangling connections). 18 golden tests. Full monorepo regression **9/9 green** (web 167, caml 116, catalog 19, core 106). *(Max-edge-length needs a live ELK layout → eyeballed on deploy, not CI-asserted.)* **Deployed to EC2.**

> **Phase 2C complete (Days 69–78):** the canvas composes architectures instead of nesting cards — a backdrop-layer engine (containers from member bounds), a flagged compose render mode, an AZ container layer, subnet roles, network-link folding (peering/TGW/VPN/DC → connectors), VPC-level networking placement, observability sidecars + active-path highlight, an architecture-layers band view, and a 4-way view system (Resource/Architecture/Executive/Network). Backlog: flat-ELK flow-rank decoupling (compose still reuses nested positions); SVG-export parity.
...
- [x] **81** ✅ (2026-06-22) **Search upgrade** — `ranking.ts` gains a per-service **keywords/aliases** haystack (weight 5 + a phrase bonus) so terminology resolves: "object storage"→S3, "nosql"→DynamoDB, "cache"→ElastiCache, "queue"→SQS, "firewall"→SG, "cdn"→CloudFront, "load balancer"→ALB/NLB… (+1 ranking test). Palette: typing shows a **flat ranked result list** (backend-ranked) with the **match bolded** + a domain tag, replacing the domain sections during search. 173 web + core green.
- [x] **82** ✅ (2026-06-22) **Templates in the sidebar** — a collapsible **◳ TEMPLATES** section lists the 6 architecture templates; clicking one **inserts the scaffold into the current canvas** via `editor.paste(template.model as fragment)` — which already remaps ids + rewires `group.parent`/`component.group`/connection endpoints (`remapFragment`) and cascades positions, so no new merge code was needed. `Palette` gains an optional `onInsertTemplate` prop. 173 web + typecheck green.
- [x] **83** ✅ (2026-06-22) **Command palette** — **⌘K / `/`** opens a keyboard-driven overlay (`CommandPalette.tsx`) to insert by name: "Create EC2", "Insert 3-Tier template" (services ranked + the 6 templates), ↑/↓/Enter/Esc. Inserts a service via `addComponent`/`addGroup` (cascaded position) or a template via `paste`. `/` is ignored while typing in a field. Synthetic containers moved to `domains.ts` (shared by palette + command palette). 173 web + typecheck green.
- [x] **84** ✅ (2026-06-22) **Context-aware suggestions** — pure `suggestFor` (`suggestions.ts`): selecting a resource surfaces the services that can connect to it (via the prefetched connection rules, either direction), curated-ordered (EC2 → Security Group · IAM Role · EBS · ALB), excluding its own service + already-connected ones, capped at 4. Rendered as a **✦ Suggested** chip row in the inspector; clicking one inserts + auto-attaches via the Day-54 path. +4 tests (177 web).
> **Sidebar UI (79–84) shipped** on the existing 61 services — no catalog dependency. Remaining 85–87 light up the 🎧/🤖 domains (→ Phase 3A).
...
- [x] **98** ✅ (2026-06-23) Rules graph + **intermediary path-finder** (`pathfinder.ts`) — `buildRulesGraph(services)` derives a directed type-graph by reusing `evaluateConnection` for every candidate edge (forward, non-`flip` only), plus a deterministic type→representative-service map (lowest key); `findIntermediaryPaths(graph, src, tgt)` does a bounded DFS (≤ 3 edges) returning the through-intermediary paths shortest-first, each step carrying type + service key + entry kind. Pure + tested: **EventBridge → S3** (the reported logical error) routes through Lambda (`messaging.eventbus → compute.serverless.function → storage.object`, async→data); storage-sink/unknown/same-type return `[]`. +8 tests (185 web). *Wires into the connect flow at Day 99 (rich verdict).*
- [x] **99** ✅ (2026-06-23) **Rich verdict** — `assessConnection(source, target, graph, nameOf)` (`pathfinder.ts`) returns a 4-state `ConnectionAssessment`: **supported** (direct rule, carries the kind), **needs-intermediary** (no direct rule but a path exists → carries `path` + `alternatives` + a "route via X" reason), **unsupported** (dead-end), with **discouraged** reserved for the Day-102 anti-pattern metadata. Wired into the canvas connect flow: `Editor.evaluate` builds the rules graph from the prefetched catalog + connection rules and now turns a rejected drag into a *named suggestion* in the hint (EventBridge → S3 → "route via Lambda") instead of a bare "invalid"; drawing still goes through the plain `verdict` (a needs-intermediary pair isn't one straight line — the **Insert** action lands Day 101). +4 tests (189 web), tsc clean. *Structured suggestion panel → Day 100.*
- [x] **100** ✅ (2026-06-23) **Recommendation panel** (`RecommendationPanel.tsx`) — a rejected connection *drop* (not just hover) now opens a structured "⚠ No direct connection" panel listing the suggested chains (`EventBridge → Lambda → S3`, numbered, best first + alternatives) each with an **Insert** button, replacing the bare hint for the needs-intermediary case. Canvas detects the rejected drop via a drag-tracked `rejectedRef` + `onConnectEnd` → new `onRejectedConnect` prop; Editor resolves the assessment and renders the panel (Resource view only, themed via `--cac-*` vars), dismissed on Insert / ✕ / selection change. The transient unsupported hint is unchanged.
- [x] **101** ✅ (2026-06-23) **Auto-correction** — `buildPathInsertion(source, target, path, serviceFor)` (`pathfinder.ts`, pure) turns a path into commands: **AddComponent** per intermediary (the existing target is never re-created) + **Connect** wiring `source → I₁ → … → target` with each step's kind; `editor.insertPath` applies them as **one undoable commit** (mirrors `addComponentLinkedTo`/`paste`), cascading the new nodes off the source. Clicking **Insert** materializes the EventBridge → Lambda → S3 fix through the normal command bus + autosave. +2 tests (191 web), tsc + production build clean.
- [x] **102** ✅ (2026-06-23) **Knowledge metadata** — catalog format schema gains an optional `connectionRules.knowledge` block (`recommendedTargets` / `requiresIntermediary` / `antiPatterns` / `recommendedPatterns`) + `ConnectionKnowledge`/`AntiPattern` TS types. Authored on the **4 highest-value services**: `aws.eventbridge` + `aws.sns` (event routers → storage/db go through a function), `aws.cloudwatch` (monitoring → storage via a Logs subscription), `aws.iam_role` (a grant isn't a data path). Lint extended with a **`dangling-knowledge`** check — every type a `knowledge` block names must be providable — and the gate passes **0 warnings / 61 services**. Flows through `getAllConnectionRules` to the API unchanged (consumed by 103/104). +2 lint tests (21 catalog), core builds clean.
- [x] **103** ✅ (2026-06-23) **Anti-pattern validation** — new advisory rule **ARC-001** (`pack.ts` `antiPatternRule(knowledgeByService)`): a **flow** connection (traffic/data/async) whose source service's curated `knowledge.antiPatterns` flags the target type yields a `medium` finding with the curated reason. Identity/peering/dependency edges are exempt (a role→resource *grant* is correct), so it won't false-positive on legitimate modeling; it's the **server-side catch** for the EventBridge→S3-style edges the canvas rejects but the API/AI can still commit (server has no connection-rules pass). Threaded via `validateModel(model, knowledgeByService?)` + `knowledgeByService(catalog)`; the architecture controller's `/validate` now goes through `service.validateBranch` (has CATALOG). Back-compatible (no map → unchanged pack). +2 tests (28 validation), core builds clean.
- [x] **104** ✅ (2026-06-23) **Architecture Advisor panel** — pure `buildAdvisor(rules, nameForType)` (`advisor.ts`) projects a service's `knowledge` into **Recommended** (target names) · **Common patterns** (humanized ids) · **Anti-patterns** (✗ reason), and the Inspector renders a **◆ Advisor** section beside ✦ Suggested (which already covers "valid targets"). Web `ConnectionRules` type extended with `knowledge`; Editor builds a type→name map and feeds the selected resource's rules in. +3 tests (194 web), tsc + production build clean.
- [x] **105** ✅ (2026-06-23) **Pattern library** — `patterns.ts`: 4 named, insertable **fragments** (smaller than full templates) — **Event → Store** (EventBridge→Lambda→S3), **Event Fan-out** (EventBridge→SNS→{SQS,Lambda}), **Alarm Fan-out** (CloudWatch→SNS→Lambda), **Assume Role** (IAM Role→Lambda) — each a flat CAML fragment inserted through the existing `paste` path (id-remap + position cascade). **`recommendedPatterns` wired:** `buildAdvisor` resolves the catalog ids against `PATTERNS_BY_ID` (only known/insertable ones shown), and the Advisor's **Common patterns** are now one-click insert chips (`editor.paste` the fragment). +6 tests (197 web — 3 pattern-library integrity + advisor updates), tsc + production build clean.
- [x] **106** ✅ (2026-06-23) Golden + deploy. **`pathfinder.golden.test.ts`** pins both reported cases over a verbatim real-catalog subgraph fixture — EventBridge→S3 routes via Lambda (matching the `event-to-store` pattern), CloudWatch→S3 via SNS→Lambda — plus supported/unsupported sentinels, determinism, and a **pattern-library tie-in**: every connection in every pattern fragment is a catalog-permitted edge with a permitted kind. **`knowledge.golden.test.ts`** (catalog) guards fixture faithfulness against the *real* YAML: the reported pairs still have no direct rule, and the 4 high-value services carry their `knowledge`. Full monorepo regression **green** (web 202, core 109 [+49 gated], catalog 23, caml), catalog `check` 0 warnings. **Deployed to EC2.**
...
> **Phase 3B complete (Days 98–106):** the canvas is now an architecture *advisor*. A rejected connection becomes a lesson + a one-click fix — the rules graph + BFS path-finder turns EventBridge→S3 / CloudWatch→S3 into "route via Lambda" (rich 4-state verdict → structured recommendation panel → auto-correction that inserts the intermediaries as one undoable commit). Curated `knowledge` metadata (recommendedTargets / requiresIntermediary / antiPatterns / recommendedPatterns) on the high-value services powers an inspector **Advisor** panel and the server-side **ARC-001** anti-pattern rule, and a named **pattern library** drops canonical scaffolds in one click. Deterministic + explainable throughout (graph + curated data, never an opaque "not allowed"). Reused — not rebuilt — `evaluateConnection`, `classifyRelationship`, `suggestFor`, the validation engine, and the `paste` path. Backlog: curated knowledge on the long tail of services (4 authored so far); Firehose/Step-Functions as path intermediaries (catalog-expansion); discouraged-verdict wiring once anti-pattern metadata covers drawable direct edges.

### Phase 3B follow-up — AWS placement/topology hardening ✅ (2026-06-23)
Triggered by a governance review of the pre-checks `test2` architecture (nested VPC, IGW
inside a subnet, S3 dropped into a VPC, SG wired to S3 — all constructs AWS cannot build but
that pre-dated the canvas guards). Encoded the hard constraints as deterministic validation:
- **Server pack (5 new rules)** — **NET-003** invalid container nesting (VPC-in-VPC, VPC-in-subnet, subnet-in-subnet) · **NET-004** gateway level (IGW/VPN inside a subnet; Transit Gateway inside a VPC) · **NET-005** NAT gateway not in a public subnet · **NET-006** regional/global service (S3/EventBridge/CloudWatch/Route53/DynamoDB/SNS/SQS/KMS/Secrets/Glue/IAM) placed inside a VPC · **SEC-007** security group attached to an unsupported target (regional service / Transit Gateway). Type-driven (no new catalog data); a `test2` repro fixture proves NET-003/004/006/SEC-007 fire together. +11 tests (39 validation); **0 false positives** on the 6 templates + seed (region/tier-grouped regional services are not in a VPC).
- **Live canvas badge** — `containment.ts` now forbids a VPC nested in anything but a region (nested-VPC / VPC-in-subnet flagged the instant it's drawn; subnet-in-subnet already was). +3 tests (web).

> Backlog (need the deferred routing/reachability model): Internet→private-subnet / Internet→RDS path validation beyond SEC-002's graph reach; "private subnet with no NAT/endpoint egress"; gateway-endpoint-target ≠ S3/DynamoDB. A catalog-declared `placement` scope would replace NET-006's hardcoded service list.

### Architecture Hub — landing-page redesign, P0 ✅ (2026-06-23)
Replaced the primitive text list (`ArchitectureList`) with a scalable **card hub** (`ArchitectureHub`) — front-end-only over existing APIs (`docs`/proposal: enterprise list à la Figma/Cloudcraft). **Cards with SVG thumbnails** (reuse `export.svg`), **validation-score chips** (lazy `/validate`, viewport-gated via `useInView`), status dots, relative time, and a `⋮` menu (working **Export** ▸ SVG/Terraform/HLD/Bundle + favorite; rename/duplicate/delete stubbed → P1). **Dashboard metrics**, instant **search** (name + description), **status filter**, **sort** (newest/oldest/name), **favorites** (★ + Favorites-only) and **recents** via localStorage (`useArchPrefs`, mirrors Day-80), a **Recent rail**, and an **empty state**. Pure `filterSortArchitectures`/`scoreFromReport`/`pushRecent` unit-tested (+10 web, 215). **Next: P1** — `PATCH`/`DELETE`/`duplicate` endpoints + `updated_at`/`category`/`tags` (migration 0003) to light up rename/duplicate/delete/archive, bulk-select, and sort-by-modified; **P2** folders + AI prompt history.

### Architecture Hub — landing redesign, P1 ✅ (2026-06-23)
The CRUD layer behind the P0 cards. **Backend:** migration `0003` adds `updated_at` (touched on every commit, backfilled to `created_at`); new endpoints **`PATCH /architectures/:id`** (rename / description / lifecycle — 409 on duplicate name, 404 if absent), **`DELETE /architectures/:id`** (cascades commits + branches → 204), **`POST /architectures/:id/duplicate`** (copies the head model into a new architecture via the shared `seed()` path); list now returns `updated_at`. OpenAPI + client regenerated. **Web:** `archActions.ts` (raw-fetch mutations) wires the `⋮` menu — **Rename** / **Duplicate** (→ opens the copy) / **Archive** / **Delete…** — with a text-prompt dialog and a name-echoing **delete confirmation** (`ArchDialogs.tsx`); **sort-by-modified is now the default**. +1 web test (216); core 120; tsc + builds clean. *Remaining for P2: bulk multi-select, folders, `category`/`tags` + search-by-tag, AI prompt history.*

### Architecture Hub — landing redesign, P2 ✅ (2026-06-24)
Shipped in four deployed slices. **(1) Bulk multi-select** — per-card checkbox + tri-state select-all; sticky bulk bar (★ Favorite · Move to… · Archive · Delete…) fanning out over the single-item endpoints via `Promise.allSettled` with partial-failure reporting; pure `selectionStats`/`pruneSelection` + `useSelection`. **(2) Tags + search-by-tag** — migration `0004` (`tags TEXT[]` + GIN); `PATCH` accepts `tags`, normalized by pure `normalizeTags` (trim/lowercase/dedupe, max 12×32); clickable tag chips + a tag-facet bar with counts; free-text search now also matches tags. ("category" folded into tags — no separate fixed taxonomy.) **(3) Folders** — migration `0005` (`folders` table + `architectures.folder_id` FK, `ON DELETE SET NULL`); `FolderController` CRUD (counts in list); `folderId` on `PATCH` (validated); folder rail (All/Unfiled/each + create/rename/delete) + single & bulk **Move to folder…** (`MoveToFolderDialog`, create-and-move). **(4) AI prompt history** — client-side `usePromptHistory` (localStorage, pure `pushPrompt`), a 🕘 History toggle in `AiConsole` re-filling the input (pipeline still stubbed → no server persistence yet). OpenAPI + client regenerated for the new fields/endpoints. Tests: web 215 → **228**, core 120 → **125**; tsc + builds clean across slices. *Hub P2 complete.*
